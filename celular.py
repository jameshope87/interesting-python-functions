...
def next_generation(string, pattern):
    string_1 = ''
    dict = make_dict(pattern)
    for i in range(len(string)):
        for e in dict:
            if e == string[i-1] + string[i] + string[((i+1)%len(string))]:
                string_1 += dict[e]
    return string_1
...
def cellular_automaton(string, pattern, n):
    if n == 0:
        ...
        return next_generation(string, pattern)
    else:
        return cellular_automaton(string, pattern, n-1)
